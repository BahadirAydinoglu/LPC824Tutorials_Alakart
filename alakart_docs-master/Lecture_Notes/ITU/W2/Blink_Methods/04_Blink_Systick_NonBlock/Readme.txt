...
This program demonstrates the use of system tick interrupt to implement a precisely timed interval in a non-blocking fashion.
...
In this example, SysTick is set to produce an INT every millisecond. The handler for the sysTick interrupt is used to count the number of milliseconds that have elapsed and tuns a LED on and off. Since all of this work is done in the background by the SysTick interrupt service routine, the main function is free to do other tasks in the foreground.
